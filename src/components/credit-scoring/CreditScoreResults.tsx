...
export function CreditScoreResults({ lambdaData, onStartNewForm }: CreditScoreResultsProps) {
  ...
  const percentage = currentScore !== null ? (currentScore / maxScore) * 100 : 0;
  ...
  const getScoreLabel = (key: string, language: string): string => {
    const labels: { [key: string]: { vi: string; en: string } } = {
      financial_responsibility: { vi: 'Trách nhiệm tài chính', en: 'Financial Responsibility' },
      stability_consistency: { vi: 'Ổn định & Nhất quán', en: 'Stability & Consistency' },
      planning_execution: { vi: 'Lập kế hoạch & Thực hiện', en: 'Planning & Execution' },
      stress_resilience: { vi: 'Khả năng chống chịu stress', en: 'Stress Resilience' },
      social_professional_responsibility: { vi: 'Trách nhiệm xã hội & nghề nghiệp', en: 'Social & Professional Responsibility' }
    };
      ...
      <CardContent className="space-y-8">
        ...
        {apiScoreData?.individual_scores && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(apiScoreData.individual_scores).map(([key, value]) => {
        ...
        {apiScoreData?.llm_details && apiScoreData.llm_details.length > 0 && (
          <div className="space-y-6">
            <h3 className="text-lg font-semibold text-gray-800 text-center">
              {language === 'vi' ? 'Giải Thích Chi Tiết Theo Tệp' : 'Detailed Explanations by File'}
            </h3>
            {apiScoreData.llm_details.map((fileDetail, fileIndex) => {
              const isExpanded = expandedFiles.has(fileIndex);
              
              return (
                <div key={fileIndex} className="border border-gray-300 rounded-lg">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-600 capitalize">
                          {language === 'vi' ? 'Loại:' : 'Type:'} {fileDetail.classification}
                        </p>
                      </div>
                      <ChevronDown 
                      ...
                      {fileDetail.key_findings && (
                        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                          <h5 className="text-sm font-medium text-yellow-800 mb-1">
                            {language === 'vi' ? 'Phát Hiện Chính' : 'Key Findings'}
                          </h5>
                          <p className="text-sm text-yellow-700 leading-relaxed">{fileDetail.key_findings}</p>
                        </div>
                      )}
                      
                      {/* Evidence Details */}
                      <div className="space-y-3 mb-4">
                        {Object.entries(fileDetail.evidence).map(([key, explanation]) => {
                          const categoryLabel = getScoreLabel(key, language);
                          const categoryScore = fileDetail.scores[key]?.score || 0;
                          const scorePercentage = Math.round((categoryScore / 5) * 100); // Convert 1-5 scale to percentage
                          const scoreColor = getScoreColor(scorePercentage);
                          const riskCategory = getRiskCategoryLabel(categoryScore / 5, language); // Convert to 0-1 scale for risk category
                          ...
                          return (
                            <div key={key} className="bg-gray-50 rounded-lg p-3">
                              ...
                              <p className="text-xs text-gray-600 leading-relaxed mb-2">{explanation}</p>
                      ...
                      {fileDetail.data_quality && (
                        <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                          <h5 className="text-sm font-medium text-blue-800 mb-1">
                            {language === 'vi' ? 'Đánh Giá Chất Lượng Dữ Liệu' : 'Data Quality Assessment'}
                          </h5>
                          <p className="text-xs text-blue-700 leading-relaxed">{fileDetail.data_quality}</p>
                        </div>
                      )}
                    </div>
                  )}
